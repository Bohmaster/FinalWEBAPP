angular
  ...
  .controller('LoginCtrl', function($scope, $rootScope, $uibModal, $state, $window, $location, SessionService, Usuario, ngFB) {
    ...
    $scope.registrar = function() {
      Usuario.create({
        name: $scope.credentials.full_name,
    ...
    $rootScope.login = function (user_credentials, fb) {
      ...
      SessionService.login($scope.credentials).then(function(response) {
        if(response.user.entity && !response.user.entity.active) {
          console.log('error');
            ...
            console.log('ACAAAAAAAAAAAAAAAAAAAAAA', response);
          ...
          if (fb) {
              $window.localStorage.setItem('user', JSON.stringify(user_credentials));
          } else {
        ...
        if(user_credentials) {
          ...
          if (err.data.error.code==="LOGIN_FAILED") {
            //ngToast.show("Error. Por favor chequeá tu usuario y contraseña.");
          }
        }
      });
       ...
       ngFB.login({scope: 'email,public_profile'}).then(
         function (response) {
           if (response.status === 'connected') {
             ...
             //ngToast.show('Error en intento de login con Facebook');
           }
         }
       );
     };
